...
export function ServiceClusterNav({ activeCluster }: ServiceClusterNavProps) {
  ...
  const hasInterior = interiorServices.length > 0;
  const hasExterior = exteriorServices.length > 0;

  // Don't render if only one cluster with nothing to navigate between
  if (!hasInterior && !hasExterior) return null;

  return (
    <nav className="py-8 bg-white border-y border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={hasInterior && hasExterior ? 'grid md:grid-cols-2 gap-8' : ''}>
          {/* Interior Cluster */}
          {hasInterior && (
            <div>
